...
export class Lexer{    
    ...
    public getNextToken(): Token{        
        ...
        if(!Accepted){
            ...
            if(AcceptingState == TypescriptFSMStates.AU){
                token = new Token(TokenTypes.AU, TokenTypes[TokenTypes.AU], AnalyzedString, this.line, this.column);
                this.position += AnalyzedString.length;
                this.column += AnalyzedString.length;
                return token;

            } 
            if(AcceptingState == TypescriptFSMStates.AV){
                token = new Token(TokenTypes.AV, TokenTypes[TokenTypes.AV], AnalyzedString, this.line, this.column);
                this.position += AnalyzedString.length;
                this.column += AnalyzedString.length;
                return token;

            } 
            if(AcceptingState == TypescriptFSMStates.AW){
                token = new Token(TokenTypes.AW, TokenTypes[TokenTypes.AW], AnalyzedString, this.line, this.column);
                this.position += AnalyzedString.length;
                this.column += AnalyzedString.length;
                return token;

            } 
            if(AcceptingState == TypescriptFSMStates.AX){
                token = new Token(TokenTypes.AX, TokenTypes[TokenTypes.AX], AnalyzedString, this.line, this.column);
                this.position += AnalyzedString.length;
                this.column += AnalyzedString.length;
                return token;

            } 
            if(AcceptingState == TypescriptFSMStates.AY){
                token = new Token(TokenTypes.AY, TokenTypes[TokenTypes.AY], AnalyzedString, this.line, this.column);
                this.position += AnalyzedString.length;
                this.column += AnalyzedString.length;
                return token;

            } 
            if(AcceptingState == TypescriptFSMStates.AZ){
                token = new Token(TokenTypes.AZ, TokenTypes[TokenTypes.AZ], AnalyzedString, this.line, this.column);
                this.position += AnalyzedString.length;
                this.column += AnalyzedString.length;
                return token;
            } 
